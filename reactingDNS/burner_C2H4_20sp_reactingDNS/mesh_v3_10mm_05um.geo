...
Point(29) = {                aux*cos, burner_height + flame_dist,                aux*sin, 0.0010};
...
aux = 0.5*(burner_base_radius + ext_radius);
Point(61) = { aux*cos, 0.18, aux*sin, 0.0015};
Point(62) = {     0.0, 0.18,     0.0, 0.0010};
...
Transfinite Line { 18} = 33 Using Progression 1.015;

Transfinite Line {  5} = 41 Using Progression 1.002;
Transfinite Line { 19} = 41 Using Progression 1.002;
...
Transfinite Line {-24} = 13 Using Progression 1.02;
Transfinite Line { 22} = 13 Using Progression 1.02;
/*vertical*/
Transfinite Line {  3} = 53 Using Progression 1.08;
Transfinite Line { 16} = 53 Using Progression 1.08;
Transfinite Line {  4} = 53 Using Progression 1.08;
Transfinite Line {  6} = 53 Using Progression 1.08;
...
//Transfinite Line { 30} = 17 Using Progression 1.041;
Transfinite Line { 30} = 15 Using Progression 1.007;
...
/*Fluid*/
Line Loop(1) = {10:11,36,37,35,41:44};
Plane Surface(1) = {1};
/*Recombine Surface(1);*/

Line Loop(2) = {12,17,18,-24,26,27,28,29,30,-37,-36};
Plane Surface(2) = {2};
Recombine Surface(2);

Line Loop(5) = {1,3,-17,-16};
Plane Surface(5) = {5};
Transfinite Surface {5} Alternate;
Recombine Surface(5);

Line Loop(4) = {4,-18,-3, 2};
Plane Surface(4) = {4};
Transfinite Surface {4} Alternate;
Recombine Surface(4);

Line Loop(3) = {-4,5,6,-19};
Plane Surface(3) = {3};
Transfinite Surface {3} Alternate;
Recombine Surface(3);

Line Loop(6) = {15,16,-12,-13};
Plane Surface(6) = {6};
Transfinite Surface {6} Alternate;
Recombine Surface(6);

Line Loop(7) = {14,13,-11,-10};
Plane Surface(7) = {7};
Transfinite Surface {7} Alternate;
Recombine Surface(7);

Line Loop(8) = {19,22,23,24};
Plane Surface(8) = {8};
...
Physical Surface("f_back") = {102,154,171,193,215,237,259,276,293,315,337,359,376};
Physical Surface("outlet") = {89};
Physical Surface("farfield") = {93,97};
Physical Surface("burner") = {101,224,246};
Physical Surface("shield") = {202};
Physical Surface("fuel") = {166,192};
Physical Surface("sample") = {284,310,336,350,375};
